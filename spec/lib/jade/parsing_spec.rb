  ...
  describe 'parse errors' do
    ...
    context 'an empty function body' do
      ...
      it 'names the missing body rather than the token it stopped on' do
        ...
        expect(err.message)
          .to eq('While parsing function declaration: This body is empty — ' \
                 'it has to produce a value (got "end" with nothing before it)')
      end

      it 'spans the gap the body should have filled, not the `end`' do
        result => Err(err)

        expect(text[err.span]).to eq("\n")
      end
    end
