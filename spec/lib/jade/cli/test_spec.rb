  ...
  module CLI
    describe Test do
      ...
      it 'reads the format off the command line' do
        ...
        expect(Test.parse(['--format=doc', 'MathTest'])).to eql [:doc, ['MathTest']]
        expect(Test.parse(['MathTest'])).to eql [:dots, ['MathTest']]
      end

      # Without this it is taken for a pattern, and the run reports no
      # matching modules rather than a missing value.
      it 'refuses a format flag with nothing after it' do
        expect { Test.parse(['-f']) }.to raise_error(SystemExit)
      end
