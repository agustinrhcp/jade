...
module Jade
  describe ModuleLoader do
    ...
    describe 'its modules in topo order' do
      ...
      it { is_expected.to eql %w[Maybe Result Decode.Params Test.Pepe Test.Path] }
    end
    ...
    describe '.emit' do
      ...
      it 'writes ruby files' do
        expect(FileUtils).to receive(:mkdir_p).exactly(5).times
