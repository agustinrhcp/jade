...
module Jade
  module Testing
    describe Runner do
      ...
      # An intrinsic that reaches Ruby can still raise, which is the crash a
      # runner has to contain. Division used to be the example here, until
      # it stopped being able to.
      it 'contains a crash to the test that caused it' do
        mod = build('CrashingTest', <<~JADE)
          describe(
            "math",
            [
              it("repeats a negative number of times", -> {
                Expect.equal(String.repeat("ab", 0 - 1), "")
              }),
        ...
        expect(crashed.error).to be_a ArgumentError
        expect(after).to be_a Passed
      end

      it 'knows a module it cannot run' do
        expect(Runner.unrunnable_reason(build('RunnableTest', 'describe("m", [])'))).to be_nil
        expect(Runner.unrunnable_reason(Module.new)).to eql Runner::MISSING
      end
      ...
      it 'explains a module whose tests never became a value' do
        ...
        expect(Runner.unrunnable_reason(mod)).to match(/did not compile to a value/)
      end
    end
  end
end
