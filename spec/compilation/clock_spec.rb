...
module Jade
  describe 'Clock' do
    ...
    it 'now() returns a Task that resolves to an Instant after the epoch' do
      ...
      expect(Use::Internal.since_epoch_ms(result._1)).to be > 0
    end

    it 'epoch is 1970-01-01T00:00:00Z' do
      expect(Use::Internal.iso(Use::Internal.epoch)).to eql '1970-01-01T00:00:00Z'
    end

    it 'add(epoch, from_millis(n)) reaches a specific Instant' do
      i = Use::Internal.at_ms(1_700_000_000_000)
      expect(Use::Internal.since_epoch_ms(i)).to eql 1_700_000_000_000
    end

    describe 'Duration unit constructors and accessors' do
      it 'from_millis / to_millis round-trip' do
        expect(Use::Internal.to_ms(Use::Internal.ms_to(500))).to eql 500
      end

      it 'from_seconds / to_seconds' do
        expect(Use::Internal.to_ms(Use::Internal.sec_to(60))).to eql 60_000
        expect(Use::Internal.to_sec(Use::Internal.ms_to(2_500))).to eql 2
      end

      it 'from_minutes / to_minutes' do
        expect(Use::Internal.to_ms(Use::Internal.min_to(2))).to eql 120_000
        expect(Use::Internal.to_min(Use::Internal.ms_to(120_500))).to eql 2
      end

      it 'from_hours / to_hours' do
        expect(Use::Internal.to_ms(Use::Internal.hr_to(1))).to eql 3_600_000
        expect(Use::Internal.to_hr(Use::Internal.ms_to(7_260_000))).to eql 2
      end

      it 'from_days / to_days' do
        expect(Use::Internal.to_ms(Use::Internal.day_to(1))).to eql 86_400_000
        expect(Use::Internal.to_day(Use::Internal.ms_to(2 * 86_400_000 + 100))).to eql 2
      end
    end
    ...
    describe 'to_parts on a Duration' do
      ...
      it 'zeros out parts smaller than the duration' do
        parts = Use::Internal.parts(duration(500))
    ...
    describe 'to_time_of_day on an Instant' do
      it 'extracts wall-clock parts from epoch millis' do
        tod = Use::Internal.time_of_day(Use::Internal.at_ms(1_700_000_000_123))
      ...
      it 'returns midnight for the epoch' do
        tod = Use::Internal.time_of_day(Use::Internal.epoch)
    ...
    describe 'add / diff' do
      it 'add(instant, duration) returns a shifted instant' do
        i = Use::Internal.at_ms(1_000_000)
        d = duration(500)
        expect(Use::Internal.since_epoch_ms(Use::Internal.advance(i, d))).to eql 1_000_500
      end

      it 'diff(a, b) returns a Duration in millis' do
        a = Use::Internal.at_ms(1_000_000)
        b = Use::Internal.at_ms(1_000_500)
        expect(Use::Internal.to_ms(Use::Internal.span(a, b))).to eql 500
      end
    end

    describe 'ISO 8601 formatting and parsing' do
      it 'formats a known epoch into an ISO string' do
        expect(Use::Internal.iso(Use::Internal.at_ms(1_700_000_000_000))).to eql '2023-11-14T22:13:20Z'
      end

      it 'parses an ISO 8601 UTC string' do
        result = Use::Internal.parse_iso('1970-01-01T00:00:00Z')
        expect(result).to be_ok
      ...
      it 'parses with a space separator (PostgreSQL style)' do
        ...
        expect(result).to be_ok
        expect(Use::Internal.since_epoch_ms(result._1)).to eql 1_700_000_000_000
      end

      it 'parses sub-second precision' do
        result = Use::Internal.parse_iso('2023-11-14T22:13:20.123Z')
        expect(result).to be_ok
        expect(Use::Internal.since_epoch_ms(result._1)).to eql 1_700_000_000_123
      end

      it 'parses zero-padded hour/minute/second fields' do
        result = Use::Internal.parse_iso('2026-05-22T09:08:07Z')
        expect(result).to be_ok
      end

      it 'returns Err for an invalid timestamp' do
        expect(Use::Internal.parse_iso('not-a-time')).to be_err
      end

      it 'projects an Instant onto a Calendar Date' do
        d = Use::Internal.to_date(Use::Internal.at_ms(1_700_000_000_000))
    ...
    describe 'Comparable / Eq on Instant' do
      it 'orders instants' do
        a = Use::Internal.at_ms(100)
        b = Use::Internal.at_ms(200)
      ...
      it 'compares for equality' do
        a = Use::Internal.at_ms(100)
        b = Use::Internal.at_ms(100)
        c = Use::Internal.at_ms(101)
        expect(Use::Internal.equal(a, b)).to be true
        expect(Use::Internal.equal(a, c)).to be false
      end
    end
    ...
    describe 'Encodable / Decodable' do
      ...
      it 'encodes an Instant as ISO 8601' do
        i = Use::Internal.at_ms(1_700_000_000_000)
        expect(Json::Internal.instant_to_json(i)).to eql '"2023-11-14T22:13:20Z"'
      end

      it 'decodes an Instant from ISO 8601' do
        result = Json::Internal.instant_from_json('"2023-11-14T22:13:20Z"')
        expect(result).to be_ok
        expect(Use::Internal.since_epoch_ms(result._1)).to eql 1_700_000_000_000
      end
      ...
      it 'decodes a Duration from Int milliseconds' do
        result = Json::Internal.duration_from_json('5000')
        expect(result).to be_ok
        expect(result._1).to eql Json::Internal.make_duration(5000)
      end
    end
  end
end
