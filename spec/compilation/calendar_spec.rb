...
module Jade
  describe 'Calendar' do
    ...
    it 'extracts the year' do
      d = Use::Internal.build.call(2026, Calendar::May[], 4)
      expect(Use::Internal.year_of.call(d)).to eql 2026
    end

    it 'extracts the month' do
      d = Use::Internal.build.call(2026, Calendar::May[], 4)
      expect(Use::Internal.month_of.call(d)).to be_may
    end

    it 'extracts the day' do
      d = Use::Internal.build.call(2026, Calendar::May[], 4)
      expect(Use::Internal.day_of.call(d)).to eql 4
    end

    it 'pattern-matches Month variants in user code' do
      expect(Use::Internal.describe_month.call(Calendar::Jan[])).to eql 'January'
      expect(Use::Internal.describe_month.call(Calendar::Dec[])).to eql 'December'
    end

    it 'compares two dates via the Comparable instance (year differs)' do
      a = Use::Internal.build.call(2025, Calendar::May[], 4)
      b = Use::Internal.build.call(2026, Calendar::May[], 4)
    ...
    it 'compares two dates via the Comparable instance (month differs)' do
      a = Use::Internal.build.call(2026, Calendar::Jan[], 31)
      b = Use::Internal.build.call(2026, Calendar::Feb[], 1)
      expect(Use::Internal.before.call(a, b)).to be true
    end

    it 'compares two dates via the Comparable instance (day differs)' do
      a = Use::Internal.build.call(2026, Calendar::May[], 4)
      b = Use::Internal.build.call(2026, Calendar::May[], 5)
      expect(Use::Internal.before.call(a, b)).to be true
    end
    ...
    it 'reports unequal dates via the Eq instance' do
      a = Use::Internal.build.call(2026, Calendar::May[], 4)
      b = Use::Internal.build.call(2026, Calendar::May[], 5)
      expect(Use::Internal.equal.call(a, b)).to be false
    end
    ...
    it 'computes weekday for a known date (2026-05-04 is a Monday)' do
      d = Use::Internal.build.call(2026, Calendar::May[], 4)
      expect(Use::Internal.weekday_of.call(d)).to be_mon
    end

    it 'computes weekday for a known date (2026-05-03 is a Sunday)' do
      d = Use::Internal.build.call(2026, Calendar::May[], 3)
      expect(Use::Internal.weekday_of.call(d)).to be_sun
    end

    it 'pattern-matches Weekday variants' do
      expect(Use::Internal.describe_weekday.call(Calendar::Mon[])).to eql 'Monday'
      expect(Use::Internal.describe_weekday.call(Calendar::Sun[])).to eql 'Sunday'
    end

    it 'formats a Date as an ISO 8601 string' do
      d = Use::Internal.build.call(2026, Calendar::May[], 4)
      expect(Use::Internal.iso.call(d)).to eql '2026-05-04'
    end

    it 'pads single-digit month and day' do
      d = Use::Internal.build.call(2026, Calendar::Jan[], 9)
      expect(Use::Internal.iso.call(d)).to eql '2026-01-09'
    end

    it 'parses an ISO 8601 string into a Date' do
      expect(Use::Internal.parse_iso.call('2026-05-04')).to be_ok(
        have_attributes(year: 2026, month: be_may, day: 4)
      )
    end

    it 'returns Err for an invalid ISO string' do
      expect(Use::Internal.parse_iso.call('not-a-date')).to be_err
    end

    it 'add(Days) advances the date' do
      d = Use::Internal.build.call(2026, Calendar::May[], 4)
      expect(Use::Internal.shift.call(d, Calendar::Days[], 10))
        .to eql Use::Internal.build.call(2026, Calendar::May[], 14)
    end

    it 'add(Days) crosses a month boundary' do
      d = Use::Internal.build.call(2026, Calendar::Jan[], 30)
      expect(Use::Internal.shift.call(d, Calendar::Days[], 5))
        .to eql Use::Internal.build.call(2026, Calendar::Feb[], 4)
    end

    it 'add(Months) advances by months' do
      d = Use::Internal.build.call(2026, Calendar::May[], 4)
      expect(Use::Internal.shift.call(d, Calendar::Months[], 3))
        .to eql Use::Internal.build.call(2026, Calendar::Aug[], 4)
    end

    it 'add(Months) clamps to month length' do
      d = Use::Internal.build.call(2026, Calendar::Jan[], 31)
      expect(Use::Internal.shift.call(d, Calendar::Months[], 1))
        .to eql Use::Internal.build.call(2026, Calendar::Feb[], 28)
    end

    it 'add(Years) advances by years' do
      d = Use::Internal.build.call(2026, Calendar::May[], 4)
      expect(Use::Internal.shift.call(d, Calendar::Years[], 5))
        .to eql Use::Internal.build.call(2031, Calendar::May[], 4)
    end

    it 'add(Years) handles leap-year clamping' do
      d = Use::Internal.build.call(2024, Calendar::Feb[], 29)
      expect(Use::Internal.shift.call(d, Calendar::Years[], 1))
        .to eql Use::Internal.build.call(2025, Calendar::Feb[], 28)
    end

    it 'diff(Days) counts whole days between dates' do
      a = Use::Internal.build.call(2026, Calendar::May[], 4)
      b = Use::Internal.build.call(2026, Calendar::May[], 14)
      expect(Use::Internal.span.call(a, b, Calendar::Days[])).to eql 10
    end

    it 'diff(Months) counts full months between dates' do
      a = Use::Internal.build.call(2026, Calendar::Jan[], 15)
      b = Use::Internal.build.call(2026, Calendar::Apr[], 14)
      expect(Use::Internal.span.call(a, b, Calendar::Months[])).to eql 2
    end
    ...
    context 'Encodable / Decodable' do
      ...
      it 'encodes a Date as an ISO 8601 string' do
        d = Use::Internal.build.call(2026, Calendar::May[], 4)
        expect(Json::Internal.date_to_json.call(d)).to eql '"2026-05-04"'
      end

      it 'decodes a Date from an ISO 8601 string' do
        expect(Json::Internal.date_from_json.call('"2026-05-04"'))
          .to be_ok(have_attributes(year: 2026, day: 4))
      end

      it 'fails decoding an invalid ISO date' do
        expect(Json::Internal.date_from_json.call('"not-a-date"')).to be_err
      end
    end
  end
end
