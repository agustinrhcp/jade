...
module Jade
  module CLI
    module Test
      ...
      def run(argv)
        ...
        parse(argv).then do |(format, patterns)|
          Jade::Project.find!.then do |project|
            discover(project, patterns).then do |uris|
              uris.empty? ? no_tests(project, patterns) : execute(project, uris, format)
            end
          end
        end
      end
      ...
      def parse(argv)
        case argv
        in [*head, ('-f' | '--format'), name, *tail]
          [format_named(name), head + tail]

        in [*head, /\A--format=(.+)\z/ => flag, *tail]
          [format_named(flag.split('=', 2).last), head + tail]

        in [*, ('-f' | '--format') => flag] then missing_format(flag)
        else [:dots, argv]
        end
      end

      def missing_format(flag)
        warn "jade test: #{flag} wants a format (#{FORMATS.keys.join(' or ')})"
        exit 1
      end

      def format_named(name)
        FORMATS.fetch(name) do
          warn "jade test: unknown format #{name.inspect} " \
            "(want #{FORMATS.keys.join(' or ')})"
          exit 1
        end
      end
      ...
      def load_one(compiler, uri)
        module_name(uri)
          ...
          .then { |(name, mod)| unrunnable(name, mod) || mod }
      rescue NameError => e
      ...
      def unrunnable(name, mod)
        Testing::Runner
          .unrunnable_reason(mod)
          &.then { Testing::Broken[name, RuntimeError.new(it)] }
      end
      ...
      def no_tests(project, patterns)
        warn <<~TXT
          jade test: no #{patterns.empty? ? '' : 'matching '}test modules under
          #{project.source_root}

          Test modules are files named `*#{SUFFIX}`. Run `jade test --help`
